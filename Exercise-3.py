...
# Функция, обновляющая собранные вакансии в имеющуюся БД.
# При необходимости, можно задавать имя БД и (или) коллекции через переменную, запрашивая у юзера,
# но захардкодим))
def update_db(data_vacancies):
    client = MongoClient('127.0.0.1', 27017)
    db = client['vacancies']
    hh = db.hh
    for data in data_vacancies:
        hh.update_one({'link': data['link']}, {'$set': data}, upsert=True)
